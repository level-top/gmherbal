...
type TagKey = "ALL" | "GHEE" | "OIL" | "PURIFIED" | "UNPURIFIED" | "KOHLU";
...
export function ShopProductsClient(props: { products: PublicProduct[] }) {
  const cart = useCart();
  ...
  const tags: Array<{ key: TagKey; label: string }> = [
    ...
    { key: "OIL", label: "Oils" },
    { key: "KOHLU", label: "Kohlu" },
    { key: "PURIFIED", label: "Purified" },
    { key: "UNPURIFIED", label: "Unpurified" },
  ];
  ...
  const filtered = useMemo(() => {
    ...
    return props.products.filter((p) => {
      if (tag === "GHEE" && p.category !== "GHEE") return false;
      if (tag === "OIL" && p.category !== "OIL") return false;
      if (tag === "KOHLU" && p.extraction !== "KOHLU_COLD_PRESS") return false;
      if (tag === "PURIFIED" && p.variant !== "PURIFIED") return false;
      if (tag === "UNPURIFIED" && p.variant !== "UNPURIFIED") return false;

      if (!q) return true;
      const haystack = `${p.name} ${p.description ?? ""} ${p.sizeLabel ?? ""}`.toLowerCase();
      return haystack.includes(q);
    });
  }, [props.products, query, tag]);

  return (
    <div className="mt-6">
      <div className="rounded-3xl border border-earth/20 bg-surface p-5 shadow-sm">
        <label className="grid gap-2">
          <span className="text-sm font-semibold text-heading">Search</span>
          <input
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            className="h-12 rounded-2xl border border-earth/20 bg-bg px-4 text-sm text-heading outline-none focus:ring-2 focus:ring-leaf"
              ...
              <div key={p.id} className="overflow-hidden rounded-3xl border border-earth/20 bg-surface shadow-sm">
                <div className="p-5">
                  <div className="relative overflow-hidden rounded-2xl border border-earth/20 bg-bg">
                    ...
                    <button
                      type="button"
                      onClick={() => cart.addItem({ productId: p.id, name: p.name, variant: v, qty: 1 })}
                  ...
                  <div className="mt-4">
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-2">
                      <button
                        type="button"
                        onClick={() => {
                          cart.addItem({ productId: p.id, name: p.name, variant: v, qty: 1 });
                    ...
                    <div className="mt-3">
                      <Link href="#buy" className="text-xs font-semibold text-muted hover:text-forest">
                        Go to checkout form
                      </Link>
                    </div>
                  </div>
